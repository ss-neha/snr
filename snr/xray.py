...
def cooling_rate(field, data):
    ...
    T = ('gas','temperature')
    nH = data[nh].v
    return nH*nH*ak.CoolFnShure(data[T].v)#.unit("erg/cm**(-3)*s**(-1)")

def net_cooling_rate(field, data):
    nh = ('gas','number_density')
    T = ('gas','temperature')
    nH=data[nh].v
    return nH*(nH*ak.CoolFnShure(data[T].v)-hrate)#.unt("erg/cm**(-3)*s**(-1)")
...
def setup_bin_field_info(Nbin=256):
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 2:
        ...
        sys.exit()
    atbs=AthenaBinaries(binarypath=os.path.join(path,baseid),path=path,version='0.2')
    ...
    for i in ilist:
        ...
        for xf,yf in zip(xflist, yflist):
            make_one_PDF(ds,xf,yf,bin_info, wflist=['vol','mass'], 
